...
```{r include=FALSE}
NTTimeline <- get_timeline("nationaltrust", n= 1000)
filename2 <- paste0("twitter-timeline-",Sys.Date(),".Rdata")
save(NTTimeline, file = filename2)
```


```{r national-trust-timeline, message=FALSE, warning=FALSE}
p <- ggplot(NTTimeline, aes(x = as.Date(created_at, format = "%d/%m/%y"), 
                            y = retweet_count, 
...
## Tweet text
The top 500 words used 5 or more times within tweet which contain keywords around blossom. 
...
res <- gtrends(c("blossom", "blossom watch", "national trust"), geo="GB", time = 'today 1-m')
plot(res)
...
res <- gtrends(c("blossom", "blossom watch"), geo="GB")
plot(res)
...
res <- gtrends(c("national trust"), geo="GB")
plot(res)
...
Click the markers to view the tweets where an image and the #blossom hashtag exist. Zoom out for the world view!

```{r echo=FALSE, fig.height=12, fig.width=12, message=FALSE, warning=FALSE}

blossomIcon <- makeIcon(
...
blossom2021 %>%
  ...
  leaflet() %>% 
      ...
      addMarkers(lng = ~bbx_long, lat = ~bbx_lat, popup = 
                   ~paste0("<center><img src='",media_url,"' width=80%; height=80%;></center><b>@", screen_name, "</b>: ",text,"<br>"), icon = blossomIcon,
                 )
```
