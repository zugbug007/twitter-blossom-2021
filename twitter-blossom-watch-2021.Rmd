...
## Users
### Top tweeters
Top twitter users that have tweeted more than 15 tweets containing the #blossom hashtag.
```{r tweets-top-users, fig.height=7, fig.width=9}
blossom2021 %>% 
  count(screen_name) %>% 
  filter(n >= 15) %>% 
  ggplot(aes(reorder(screen_name, n), n)) + geom_col(fill = "skyblue3") + coord_flip() + 
    labs(x = "Screen Name", y = "Tweets", title = "#blossom tweets by user", subtitle = "users with >= 15 tweets") + 
    theme(axis.text = element_text(size = 12), axis.title = element_text(size = 12))
```
...
cat("This process took",monitorEndTime_blossom - monitorStartTime_blossom,"minutes to run.",sep=" ")
```
