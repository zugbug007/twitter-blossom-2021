...
# Tweet text
The 250 words used 3 or more times. 
...
blossom2021 %>% 
  ...
  count(word) %>% 
  with(wordcloud(word, n, max.words = 250, min.freq = 3, colors = brewer.pal(8, "Dark2")))
```
...
s_usable <- iconv(sentiment, 'UTF-8', 'ASCII')
...
ggplot(subset_tweets_long, aes(x = emotion, y = sentiment,
  ...
  geom_jitter(color = "#ffffff", shape = 25, size = 2, alpha = .9, stroke = .2) +
  coord_flip() + labs(y = "", x = "",
