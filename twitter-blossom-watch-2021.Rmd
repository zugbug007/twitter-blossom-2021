...
```{r}
blossom2021 <- search_tweets("#blossom OR #blossomwatch OR #NationalTrust OR #EveryoneNeedsNature", 5000)
```
...
blossom2021 %>% 
  ...
  summarise(count = n()) %>% 
  filter(date >= as_date("2021-03-11"), date <= as_date("2021-03-18")) %>%
...
blossom2021 %>% 
  ...
  ggplot(aes(reorder(screen_name, n), n)) + geom_col(fill = "skyblue3") + coord_flip() + 
    labs(x = "Screen Name", y = "Tweets", title = "#blossom tweets by user", subtitle = "users with >= 20 tweets") + 
    theme(axis.text = element_text(size = 12), axis.title = element_text(size = 12))
```
